...
public class BookAdapter {
    ...
    public long getAverageNumberOfComments(){
        long result = -1;
        String query = "SELECT avg(commentCount)" +
                " FROM " +
                "   (SELECT count(comments.id) AS commentCount FROM " + TABLE + " INNER JOIN comments ON books.id = comments.bookId GROUP BY comments.id)" +
                " AS result";
